...
{
    public const int NoticeVersion = 7;
    ...
    public static string PrivacyNotice =>
        ...
        "• Shared free account (website + Windows app): one Supabase project (degoogle). The same email/password works in both places. We store your email, a hashed password, your plan, your guide checklist, trial start time, linked license, household invites, and support messages you sent. Export and delete use the same database functions on the website (/account) and in this app (Privacy).\n" +
        ...
        "Retention: local data until you erase it or uninstall. Account data until you delete the account (rows cascade; support messages and invites cleaned up). Stripe keeps payment records under its own retention. GitHub may log download IPs under its terms.\n\n" +
        ...
        "Your rights (Arts. 15–21): access, rectification, erasure, restriction, portability, and objection. Export and delete work the same on the website account page and in this app (Privacy → Export my data / Delete my cloud account). Local files need a separate erase on this PC. You can withdraw Cloud AI consent on the Coach tab. Complaints: Datatilsynet (Denmark) " + DatatilsynetUrl + " or your local EEA authority. You may also contact us first via GitHub issues.\n\n" +
    ...
    public static string Terms =>
        ...
        "A free account is optional and shared between this app and the website. It backs up plan and checklist only. You may delete it any time in the app or on " + WebsiteAccountUrl + " — both use the same database.\n\n" +
        "Lifetime Pro (" + LicenseService.LifetimePrice + " once) and Household / Family (" + LicenseService.FamilyPrice + ", up to " + LicenseService.FamilySeats + " PCs) are one-time digital licenses sold through Stripe Checkout (website checkout page preferred). Household is the same Pro extras for several Windows PCs in one home (one key per purchase). Cards never enter this app. Cloud Pass (" + LicenseService.CloudMonthly + " or " + LicenseService.CloudYearly + ") is hosted DeGoogle AI on Groq GPT-OSS 120B. It is not billed until the live Stripe catalog is on.\n\n" +
        "EU/EEA consumer withdrawal (forbrugeraftaleloven / Directive 2011/83/EU): you normally have 14 days to withdraw from a distance contract. Lifetime Pro / Household are digital content. At checkout you must tick that you want immediate delivery of the license key and that you understand the 14-day right may be lost once the key is delivered and activated. If you have not activated the key, contact us within 14 days with subject “Withdrawal (consumer)” for a refund via Stripe. The Pro trial is " + LicenseService.TrialDays + " days, no card, and requires a signed-in free account so the trial can follow that account.\n\n" +
